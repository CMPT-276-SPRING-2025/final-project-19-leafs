document.addEventListener('DOMContentLoaded', function () {
    ...
    const displayResults = (flights) => {
        ...
        flights.forEach(flight => {
            ...
            const duration = flight.legs[0].scheduledLegDuration.replace('PT', '').toLowerCase();

            // Determine flight status
            let flightStatus = "On Time";
            let statusColor = "green"; // Default to green for "On Time"

            if (departure.departure.timings[0].delays && departure.departure.timings[0].delays.length > 0) {
    ...
    form.addEventListener('submit', async (e) => {
        ...
        const flights = await fetchFlightStatus(carrierCode, flightNumber, departureDate);
